...
\begin{tikzpicture}
	...
	\tkzTabLine{z, - , z , +, z}
	\tkzTabVar{+/$1$, -/$0$, +/$1$ }
	\tkzTabVal{1}{2}{0.5}{-$\frac{\pi}{4}$}{$\frac{1}{2}$}
	\tkzTabVal{2}{3}{0.5}{$\frac{\pi}{4}$}{$\frac{1}{2}$}
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue,samples=81]{sin(deg(x))^2};
	\end{axis}	
\end{tikzpicture}

\subsubsection*{Exercice 4.9.2}

$f_2(x) = e^{-x}\sin^2(x)$

$f_2$ est positive et oscille entre l'axe des abscisses et la courbe de $e^{-x}$
et tend donc vers $0$ en $+\infty$.

Elle touche l'axe des abscisses tous les $\pi$ et la courbe supérieure en $\frac{\pi}{2} +\pi\mathbb{Z}$

$f_2'(x) = e^{-x} \sin(x) (2\cos(x)-\sin(x))$

Les extrema inférieurs (jonctions avec l'axe des abscisses) sont $\pi\mathbb{Z}$, les extrema supérieurs
sont $\arctan(2) +\pi\mathbb{Z}$
...
\begin{tikzpicture}
	\begin{axis}[
		...
		ymax=160,
		domain=-7.5:3.5,
		xtick={-6.28, -4.71, -3.14,-1.57,0,1.57,3.14},
		xticklabels={$-2\pi$,$-\frac{3\pi}{2}$,$-\pi$, $-\frac{\pi}{2}$,$0$,$\frac{\pi}{2}$,$\pi$}
		]
		\addplot[blue,samples=101]{exp(-x)*sin(deg(x))^2};
		\addplot[gray,samples=81]{exp(-x)};
	\end{axis}	
\end{tikzpicture}

\subsubsection*{Exercice 4.9.3}
...
\begin{tikzpicture}
	\tkzTabInit{$x$ / 1 , $f'(x)$ / 1, $f(x)$/2}
	           {$-\pi$, $-\frac{2\pi}{3}$ ,$0$, $\frac{2\pi}{3}$, $\pi$}
	\tkzTabLine{z,-  , z ,+ , z, -, z,+ ,z}
	\tkzTabVar{+/$-1$, -/$-\frac{3}{2}$, +/$3$, -/$-\frac{3}{2}$, +/$-1$ }
\end{tikzpicture}
