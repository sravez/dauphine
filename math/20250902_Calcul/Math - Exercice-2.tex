...
\begin{document}
%\include{../../templates/first_pages/first_page}
\input{../../templates/first_pages/first_page}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue,domain=0:8,samples=161]{x^(1/3))};
	\end{axis}	
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue,domain=-8:0,samples=161]{-(-x)^(1/3))};
		\addplot[blue,domain=0:8,samples=161]{x^(1/3))};
	\end{axis}	
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue,domain=0.1:16,samples=161]{x^(-1/4))};
	\end{axis}	
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue, domain=0:0.9, samples=81]{exp(1/ln(x))};
		\addplot[blue, domain=1.5:5, samples=51]{exp(1/ln(x))};
	\end{axis}	
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue, samples=101]{1/(1+exp(-x))};
	\end{axis}	
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue, samples=101]{2*x*sqrt(1-x*x)};
	\end{axis}	
\end{tikzpicture}
...
\begin{displaymath}
	f_3'(x) = \frac{x+1-(x-1)}{(x+1)^2} \exp\left( \frac{x-1}{x+1}\right)
	        = \frac{2}{(x+1)^2} \exp\left( \frac{x-1}{x+1}\right) > 0
\end{displaymath}

\begin{displaymath}
	f_3''(x) = \left(\frac{-4(x+1)}{(x+1)^4} + \frac{4}{(x+1)^4}\right) \exp\left( \frac{x-1}{x+1}\right)
	         = \frac{-4x}{(x+1)^4} \exp\left( \frac{x-1}{x+1}\right)
\end{displaymath}

$f_3$ présente un point d'inflexion en $(0 ; \frac{1}{e})$

\begin{tikzpicture}
	\tkzTabInit{$x$/1, $f'$/1, $f$/2 }
	{$-\infty$, $-1$, $+\infty$}
	\tkzTabLine{, + , d , + }
	\tkzTabVar{-/$e$, +DC-/$+\infty$/$0$, +/$e$ }
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\addplot[blue, domain=-10:-2.5, samples=51]{exp((x-1)/(x+1))};
		\addplot[blue, domain=-1:9, samples=51]{exp((x-1)/(x+1))};
		\addplot[red, thin, domain=-10:9]{exp(1)};
		\addplot[red] coordinates {(-1,0)(-1,10)};
	\end{axis}	
\end{tikzpicture}
